...
export const initIO = (httpServer: Server): SocketIO => {
  ...
  if (process.env.NODE_ENV === 'production' && !process.env.BACKEND_URL?.startsWith('https')) {
    logger.warn('⚠️ EL SERVIDOR ESTÁ EN PRODUCCIÓN, PERO NO ESTÁ USANDO HTTPS. RECOMENDAMOS CONFIGURAR SSL PARA MAYOR SEGURIDAD.');
  }  
  ...
  io.use((socket, next) => {
    try {
      ...
      logger.warn(`[LIBS/SOCKET.TS] ERROR DURANTE LA AUTENTICACIÓN DEL SOCKET: ${error?.message}`);
      next(new Error("Authentication error"));
    }
  });

  io.on("connection", async socket => {
    logger.info(`👥 CLIENTE CONECTADO: ${socket.id} DESDE ${socket.handshake.address}`);
    ...
    if (!userId || userId === "undefined" || userId === "null") {
      logger.info("ONCONNECT: FALTANDO O USUARIOID INVÁLIDO");
      socket.disconnect();
      return io;
    }
